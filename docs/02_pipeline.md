...
**`system` 是提示词，不是训练目标。** 具体说：

| 阶段 | system 的处置 |
|---|---|
| 训练时拼 prompt | 按对话模板拼进去，**是模型的输入上下文** |
| 计算 loss | **被 mask 掉，不参与 loss**。标准 SFT 只在 assistant 回复上算 loss |
| 效果 | 模型学到的是"**在这段 system 的条件下**该怎么答" |

所以两条实践后果：

1. **推理时必须用同一套 system**。训练时用"只输出 JSON、不附加任何解释"教出来的
   契约遵循能力，推理时换一句措辞或者干脆不给 system，行为就会漂 ——
   模型很可能又开始在 JSON 后面加解释。
2. **system 按输出格式分了四套，推理时要按任务取对应的那条**。
   构建时会把它们落盘到 `data/vqa/system_prompts.json`：

```json
{
  "by_output_format": {"json_only": "...", "text": "...", ...},
  "task_to_output_format": {"grounding_all": "json_only", ...}
}
```

部署时按你要让模型做的任务查表取 system，别自己重写。

**如果不想让 system 进训练**：`configs/build.yaml` 里 `export.with_system: false`，
导出的条目就不带 system 字段。代价是模型没法通过 system 区分"这次要纯 JSON
还是要自然语言" —— 四种输出格式的契约就只能靠问法本身去暗示，可靠性明显下降。
**不建议关**。
...
|---|---|---|
| 每任务不同问法数（最少的） | **12** | ≥ 30 |
| 排除 `[]` 后 top20 答案占比 | **31%** | ≤ 5% |

种子模板已手工扩到 184 条，但靠手写到不了目标。**这是模板法的天花板**，
必须用大模型补：

1. `scripts/gen_question_bank.py` 扩问法（一次性，十几次调用）；
2. `vqa/llm.py` 的答案改写层（保持事实不变，改句式/顺序/详略），
   改完做事实指纹比对 + 重跑质检。

`[]` 本身占 16%，那是负样本的**正确答案**不算复读，所以统计里单列了
`top20_share_excl_empty` 这个才是真实复读率。

构建日志会直接把不达标的任务列出来，不用自己去翻 stats。
